...
export default function MerchantPortalPage() {
  ...
  useEffect(() => {
    async function fetchMerchant() {
      try {
        ...
        if (!response.ok) {
          console.warn('Failed to fetch merchant data:', response.status);
          return;
        }
        const data = await response.json();
        const name = data?.data?.merchant?.name || data?.data?.merchant?.businessName;
        if (name) {
          setMerchantName(name);
        } else {
          console.warn('Merchant name not found in response');
        }
      } catch (err) {
  ...
  async function handleLookup(e: React.FormEvent) {
    ...
    router.push(`/recipient/${slug}/${today}/${paddedOrder}`);
  }
  ...
  async function handleCreatePayment(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await response.json();
      router.push(`/recipient/${data.data.linkId}`);
    } catch (err: any) {
